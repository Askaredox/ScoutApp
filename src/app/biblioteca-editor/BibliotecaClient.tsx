...
function get_breadcrumb(
  ...
  setUrl: React.Dispatch<React.SetStateAction<{ path: string; id: string }[]>>,
) {
  const result = [];
  result.push(
    <li className="inline-flex items-center" key={0}>
      <button
        onClick={() => setUrl([{ path: "/", id: "0" }])}
        className="inline-flex items-center cursor-pointer font-medium text-gray-500 hover:text-blue-600 hover:bg-gray-500 rounded-full px-2 dark:text-gray-400 dark:hover:text-white"
      >
        <svg
    ...
    if (i === url.length - 1) {
      ...
      result.push(
        <li className="inline-flex items-center" key={i}>
          ...
          <button
            onClick={() => setUrl(url.slice(0, i + 1))}
            className="inline-flex items-center cursor-pointer font-medium text-gray-500 hover:text-blue-600 hover:bg-gray-500 rounded-full px-2 dark:text-gray-400 dark:hover:text-white"
          >
            <span className="ms-1 font-medium text-gray-500 md:ms-2 dark:text-gray-400">
              {url[i].path}
...
export default function AdminBiblioteca() {
  ...
  const [newFolderCanDownload, setNewFolderCanDownload] =
  ...
  function update_folders() {
    ...
    request(
      "GET",
      "/folder?folder=" + url[url.length - 1].id,
      "application/json",
    )
      .then((folder_data) => {
  ...
  async function newFolder(e: React.FormEvent<HTMLFormElement>) {
    ...
    await request(
      ...
      "application/json",
      JSON.stringify({
        id_parent_folder: "FOLDER#" + parent_id,
        name: newFolderName,
  ...
  async function view_file(file_data: File_data) {
    window.open("/file/" + file_data.id.split("#")[1], "_blank");
  ...
  async function deleteFolder(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    request(
      "DELETE",
      "/folder?id_folder=" + deleteId.split("#")[1],
      "application/json",
    ).then(() => {
          ...
          <section className="size-full bg-gray-50 dark:bg-gray-900 p-3 sm:p-5 antialiased ">
            <div className="px-2">
              ...
              <TableCrumbs
                ...
                dataRowsFolders={folders.map((folder, i) => (
                  <tr
                    key={i + 1}
                  ...
                  >
                    ...
                    <td
                      scope="row"
                      className="w-2/3 px-2 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white cursor-pointer"
                      onClick={() =>
                        setUrl(
                          url.concat({
                            path: folder.name,
                    ...
                    <td
                      scope="row"
                      className="w-2/3 px-2 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white"
                    ></td>
                    <td className="px-4 py-3 text-right">
                      <div className="relative inline-block text-left">
                        ...
                        >
                          ...
                          <div className="hidden lg:inline">Eliminar</div>
                        </button>
                      </div>
                    </td>
                  </tr>
                ))}
                dataRowsFiles={files.map((file, i) => (
                  <tr
                    key={i + 1}
                    className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-700"
                  >
                    ...
                    <td
                      scope="row"
                      className="w-4/5 px-2 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white cursor-pointer"
                      onClick={async () => await view_file(file)}
                    >
                      {file.name}
                    </td>
                ...
                cardFolders={folders.map((folder, i) => (
                  <FolderCard
                    key={i}
                    title={folder.name}
                    onClick={() =>
            ...
            <div className="mb-6">
              <label className="inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  onChange={() => {
                    setNewFolderCanView(!newFolderCanView);
                  }}
                  className="sr-only peer"
                  checked={newFolderCanView}
                ></input>
            ...
            <div className="mb-6">
              <label className="inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  onChange={() => {
                    setNewFolderCanDownload(!newFolderCanDownload);
                  }}
                  className="sr-only peer"
                  checked={newFolderCanDownload}
                ></input>
                <div className="relative w-11 h-6 bg-gray-400 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-600 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                <span className="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">
                  Descargable
              ...
              >
                {newFileD === null ? (
                  ...
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                      <span className="font-semibold">{newFileD.name}</span>
                    </p>
                  </div>
                )}
            ...
            <div className="my-4">
              <label className="inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  onChange={() => {
                    setNewFileCanView(!newFileCanView);
                  }}
                  className="sr-only peer"
                  checked={newFileCanView}
                ></input>
                <div className="relative w-11 h-6 bg-gray-400 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-600 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                <span className="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">
                  Visible
            ...
            <div className="my-4">
              <label className="inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  onChange={() => {
                    setNewFileCanDownload(!newFileCanDownload);
                  }}
                  className="sr-only peer"
                  checked={newFileCanDownload}
                ></input>
                <div className="relative w-11 h-6 bg-gray-400 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-600 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                <span className="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">
                  Descargable
          ...
          <div className="relative w-full max-w-2xl mx-auto p-4">
            <div className="relative rounded-2xl bg-white shadow-xl dark:bg-gray-900 px-6 py-8">
              ...
              <p className="mb-4 text-gray-500 dark:text-gray-300">
                ¿Seguro que desea eliminar este Folder?
              </p>
